    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold tracking-tight text-white/90">Resume Builder</h1>
          <p className="text-white/40">Build your professional resume step by step</p>
        </div>
        ...
        <div>
          ...
          {activeTab === "experience" && (
            <div className="space-y-4">
              {resume.experience.map((exp, i) => (
                <div key={i} className="liquid-glass rounded-2xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => removeExperience(i)} className="h-8 w-8 flex items-center justify-center rounded-lg text-red-400/60 hover:text-red-400 hover:bg-red-500/10 transition-colors">
              ...
              <button onClick={addExperience} className="liquid-glass rounded-xl w-full py-2.5 text-sm font-medium text-white/50 hover:text-white/70 transition-colors inline-flex items-center justify-center gap-2">
          ...
          {activeTab === "education" && (
            <div className="space-y-4">
              ...
              <button onClick={addEducation} className="liquid-glass rounded-xl w-full py-2.5 text-sm font-medium text-white/50 hover:text-white/70 transition-colors inline-flex items-center justify-center gap-2">
          ...
          {activeTab === "skills" && (
            <div className="space-y-4">
              ...
              <button disabled={generating} onClick={async () => {
                ...
                if (result) { const newSkills = result.split(",").map((s: string) => s.trim()).filter(Boolean); setResume((p) => ({ ...p, skills: Array.from(new Set([...p.skills, ...newSkills])) })); toast.success("Skills suggested!"); }
              }} className="liquid-glass rounded-xl px-4 py-2.5 text-sm font-medium text-white/50 hover:text-white/70 transition-colors inline-flex items-center gap-2 disabled:opacity-50">
                {generating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />} Suggest More Skills
              </button>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button disabled={generating} onClick={generateSummary} className="text-xs text-white/40 hover:text-white/60 transition-colors inline-flex items-center gap-1 disabled:opacity-50">
